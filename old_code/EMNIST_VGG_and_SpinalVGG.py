...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch', default=15, type=int, help='')
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            print(
                'Training Accuracy {}: {} % Best: {} %'.format(args.model, 100 * correct2 / total2,
                                                                           100 * best_accuracy2))
            correct2 = 0
            total2 = 0
    ...
    with torch.no_grad():
        ...
        for i in range(math.ceil(len(validation_feature) / batch_size_test)):
            ...
            reshape_size = min(batch_size_test, len(validation_feature[begin:end]))
            shape = get_dataset_shape(args.dataset)
            images = validation_feature[begin:end].reshape(reshape_size, shape[0],  shape[1],  shape[2]).type(torch.FloatTensor).to(device)
            labels = validation_target[begin:end].to(device)
        ...
        print("Test accuracy of best model on local test set: ", correct2 / total2)
